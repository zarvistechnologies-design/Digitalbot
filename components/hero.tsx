...
const sampleWaveform = [34, 58, 42, 76, 52, 88, 45, 68, 36, 80, 54, 92, 48, 72, 40, 84, 56, 66, 38, 74, 50, 90, 44, 62]
...
export default function Hero() {
    ...
    const toggleSampleAudio = (index: number, src: string) => {
        const currentAudio = sampleAudioRef.current

        if (activeSampleIndex === index && currentAudio) {
            if (currentAudio.paused) {
                currentAudio.play().catch((error) => console.error('Error playing sample:', error))
            } else {
                currentAudio.pause()
            }
            return
        ...
        audio.onloadedmetadata = () => {
            setSampleDuration(audio.duration || 0)
        }
        audio.ontimeupdate = () => {
            setSampleCurrentTime(audio.currentTime || 0)
        }
        audio.onplay = () => setIsSamplePlaying(true)
        audio.onpause = () => setIsSamplePlaying(false)
        audio.onended = () => {
    ...
    const seekSampleAudio = (time: number) => {
        if (!sampleAudioRef.current) return
        sampleAudioRef.current.currentTime = time
                                ...
                                <div className="flex flex-1 flex-col overflow-hidden rounded-[24px] sm:rounded-[36px] bg-white px-3 pb-3 pt-6 text-slate-900 sm:px-6 sm:pb-6 sm:pt-11">
                                    <div className="-mx-3 -mt-6 rounded-t-[24px] bg-white px-3 pb-2 pt-6 text-slate-900 sm:-mx-6 sm:-mt-11 sm:rounded-t-[36px] sm:px-6 sm:pb-2 sm:pt-11">
                                        ...
                                        <div className="mt-2 flex items-center justify-center gap-2 sm:mt-3">
                                            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-orange-500 sm:h-6 sm:w-6">
                                                <Bot className="h-3 w-3 text-white sm:h-3.5 sm:w-3.5" />
                                            </span>
                                            <span className="text-[9px] font-bold sm:text-xs">AI Voice Agent</span>
                                        </div>
                                    </div>

                                    <div className="mt-5 text-center sm:mt-8">
                                        <p className="text-xl font-light leading-none text-[#075e54] sm:text-3xl">Ready</p>
                                    </div>

                                    <div className="mt-4 rounded-2xl border border-orange-200/70 bg-[#fff8ed] p-2 shadow-sm sm:mt-8 sm:rounded-[20px] sm:p-4">
                                        <div className="flex items-start gap-2.5 sm:gap-3">
                                            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-orange-500 sm:h-10 sm:w-10">
                ...
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
                        {services.slice(0, 4).map((service, i) => {
                            ...
                            const isPlaying = isActive && isSamplePlaying
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 z-20 p-3 sm:p-4">
                                        ...
                                        <div className="mt-2">
                                            <div className="flex h-3.5 w-[72px] items-end justify-start gap-[1.5px]" aria-hidden="true">
                                                {sampleWaveform.map((height, barIndex) => {
                                                    const isFilled = isActive && barIndex / sampleWaveform.length <= progress
                                                    return (
                                                        <span
                                                            key={barIndex}
                                                            className={`w-[1.5px] rounded-full transition-colors ${isFilled ? 'bg-orange-300' : 'bg-white/45'}`}
                                            ...
                                            <input
                                                type="range"
                                                min="0"
                                                max={isActive && sampleDuration > 0 ? sampleDuration : 0}
                                                step="0.01"
                                                value={isActive ? sampleCurrentTime : 0}
                                                onChange={(event) => seekSampleAudio(Number(event.target.value))}
                                                disabled={!isActive || sampleDuration <= 0}
                                                aria-label={`${service.title} audio timeline`}
                                                className="mt-1 h-1 w-full cursor-pointer accent-orange-400 disabled:cursor-default"
                                            />
                                            <div className="mt-1 flex items-center justify-between text-[9px] font-medium text-white/80">
                                                <span>{formatAudioTime(isActive ? sampleCurrentTime : 0)}</span>
                                                <span>{formatAudioTime(isActive ? sampleDuration : 0)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
