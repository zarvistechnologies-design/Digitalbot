  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <button
        ...
        className="fixed left-4 top-4 z-50 rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm md:hidden"
        aria-label="Toggle navigation"
      >
        {sidebarOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
      </button>
      {sidebarOpen && <button className="fixed inset-0 z-30 bg-slate-950/45 md:hidden" onClick={() => setSidebarOpen(false)} aria-label="Close navigation" />}
      <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 w-60 transition-transform duration-300 md:translate-x-0`}>
      ...
      <main className="min-h-screen md:ml-60">
        <header className="border-b border-slate-200 bg-white/95 px-5 py-5 backdrop-blur sm:px-7 lg:px-10">
          <div className="mx-auto flex max-w-[1500px] flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {actions && <div className="flex flex-wrap items-center gap-2 pl-12 sm:pl-0">{actions}</div>}
          </div>
        </header>
