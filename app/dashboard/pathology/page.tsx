...
import {
  ...
  Plus, RefreshCw, Search, Send, Stethoscope, TestTube2, Upload, UserRound,
  Users, WalletCards, X,
} from "lucide-react";
...
type Conversation = { conversationId: string; phone: string; metaPhoneNumberId: string; patientName?: string; latestMessage: string; latestAt: string; messageCount: number; handoffStatus: "ai" | "needs_human" | "human"; handoffReason?: string; humanUntil?: string | null };
...
export default function PathologyDashboardPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const conversationRows: Conversation[] = chatRows.data.data;
      setOverview(summary.data.data); setOrders(orderRows.data.data); setPatients(patientRows.data.data); setTests(testRows.data.data); setReferrals(referralRows.data.data); setConversations(conversationRows);
      window.dispatchEvent(new CustomEvent("pathology-attention-count", { detail: conversationRows.filter(row => row.handoffStatus === "needs_human").length }));
  ...
  async function updateConversationHandoff(action: "takeover" | "resume" | "resolve") {
    if (!selectedConversation) return;
    setBusy(selectedConversation.conversationId);
    try {
      await pathologyAPI.updateHandoff(selectedConversation.phone, { action, metaPhoneNumberId: selectedConversation.metaPhoneNumberId });
      setSelectedConversation(current => current ? { ...current, handoffStatus: action === "takeover" ? "human" : "ai", handoffReason: "", humanUntil: action === "takeover" ? new Date(Date.now() + 15 * 60_000).toISOString() : null } : null);
      flash("ok", action === "takeover" ? "Human handling enabled for 15 minutes" : action === "resolve" ? "Conversation resolved and returned to AI" : "Conversation returned to AI");
      await loadAll();
    } catch (error: any) { flash("error", error.response?.data?.error || "Could not update conversation handoff"); }
    finally { setBusy(""); }
  }
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900">
    ...
    <main className="lg:ml-64">
      ...
      <div className="mx-auto max-w-[1500px] p-4 lg:p-8">
        ...
        {loading && !overview ? <div className="flex h-80 items-center justify-center"><RefreshCw className="h-7 w-7 animate-spin text-teal-700" /></div> : <>
          ...
          {tab === "tests" && <TestsView tests={tests} onAdd={() => setShowTest(true)} onToggle={async test => { await pathologyAPI.updateTest(test._id, { active: !test.active }); await loadAll(); }} />}
          {tab === "referrals" && <ReferralsView referrals={referrals} onAdd={() => setShowReferral(true)} />}
          {tab === "inbox" && <InboxView conversations={conversations} selected={selectedConversation} messages={messages} busy={busy} onSelect={openConversation} onHandoff={updateConversationHandoff} onSent={async () => { if (selectedConversation) { setSelectedConversation(current => current ? { ...current, handoffStatus: "human", humanUntil: new Date(Date.now() + 15 * 60_000).toISOString() } : null); await openConversation(selectedConversation); } await loadAll(); }} onError={text => flash("error", text)} />}
    ...
    {showTest && <TestModal onClose={() => setShowTest(false)} onSaved={async () => { setShowTest(false); flash("ok", "Test added to catalog"); await loadAll(); }} />}
...
function InboxView({ conversations, selected, messages, busy, onSelect, onHandoff, onSent, onError }: { conversations: Conversation[]; selected: Conversation | null; messages: Message[]; busy: string; onSelect: (row: Conversation) => void; onHandoff: (action: "takeover" | "resume" | "resolve") => Promise<void>; onSent: () => Promise<void>; onError: (text: string) => void }) {
  ...
  return <div className="grid min-h-[650px] overflow-hidden rounded-lg border border-slate-200 bg-white lg:grid-cols-[340px_minmax(0,1fr)]"><div className="border-r border-slate-200"><div className="border-b border-slate-200 px-4 py-3"><h2 className="font-bold">Patient conversations</h2><p className="text-xs text-slate-500">Needs-human chats appear first</p></div><div className="max-h-[590px] overflow-y-auto">{conversations.map(row => <button key={row.conversationId} onClick={() => void onSelect(row)} className={`w-full border-b border-slate-100 p-4 text-left hover:bg-slate-50 ${selected?.conversationId === row.conversationId ? "bg-green-50" : ""}`}><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-100 text-green-700"><UserRound className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="min-w-0 flex-1 truncate text-sm font-semibold">{row.patientName || `+${row.phone}`}</p>{row.handoffStatus === "needs_human" && <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold text-rose-700">Needs human</span>}{row.handoffStatus === "human" && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">Human</span>}</div><p className="truncate text-xs text-slate-500">{row.latestMessage}</p><p className="mt-1 text-[11px] text-slate-400">{row.messageCount} messages · {dateTime(row.latestAt)}</p></div></div></button>)}{!conversations.length && <Empty icon={MessageCircle} text="No WhatsApp conversations yet" />}</div></div><div className="flex min-h-0 flex-col bg-slate-50">{selected ? <><div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-5 py-3"><div className="min-w-0 flex-1"><p className="font-semibold">{selected.patientName || `+${selected.phone}`}</p><p className="text-xs text-slate-500">+{selected.phone}</p></div>{selected.handoffStatus === "needs_human" && <><span className="text-xs font-semibold text-rose-700">Needs human{selected.handoffReason ? `: ${selected.handoffReason}` : ""}</span><button disabled={busy === selected.conversationId} onClick={() => void onHandoff("resolve")} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold">Resolve</button><button disabled={busy === selected.conversationId} onClick={() => void onHandoff("takeover")} className="rounded-md bg-amber-500 px-3 py-1.5 text-xs font-semibold text-white">Take over</button></>}{selected.handoffStatus === "human" && <><span className="text-xs font-semibold text-amber-700">Human handling · AI paused</span><button disabled={busy === selected.conversationId} onClick={() => void onHandoff("resume")} className="rounded-md bg-teal-700 px-3 py-1.5 text-xs font-semibold text-white">Return to AI</button></>}{selected.handoffStatus === "ai" && <button disabled={busy === selected.conversationId} onClick={() => void onHandoff("takeover")} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold">Take over</button>}</div><div className="flex-1 space-y-3 overflow-y-auto p-5">{messages.map(message => <div key={message._id} className={`flex ${message.sentBy === "patient" ? "justify-start" : "justify-end"}`}><div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm shadow-sm ${message.sentBy === "patient" ? "border border-slate-200 bg-white" : "bg-green-600 text-white"}`}><p className="whitespace-pre-wrap">{message.message || message.documentName || `[${message.type}]`}</p><p className={`mt-1 text-right text-[10px] ${message.sentBy === "patient" ? "text-slate-400" : "text-green-100"}`}>{dateTime(message.createdAt)}</p></div></div>)}</div><form onSubmit={submit} className="flex gap-2 border-t border-slate-200 bg-white p-4"><input value={text} onChange={event => setText(event.target.value)} placeholder="Reply to patient (AI pauses for this chat)" className="h-10 flex-1 rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-green-600" /><button disabled={sending || !text.trim()} className="inline-flex h-10 items-center gap-2 rounded-md bg-green-600 px-4 text-sm font-semibold text-white disabled:opacity-50"><Send className="h-4 w-4" /> Send</button></form></> : <div className="flex flex-1 items-center justify-center"><Empty icon={MessageCircle} text="Select a patient conversation" /></div>}</div></div>;
}
...
function TestModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState({ code: "", name: "", category: "Imaging", sampleType: "Imaging", homeCollectionAllowed: false, price: "", priceType: "fixed", turnaroundHours: "", preparation: "" }); const [error, setError] = useState("");
  async function submit(event: FormEvent) { event.preventDefault(); try { await pathologyAPI.createTest({ ...form, price: Number(form.price), turnaroundHours: form.turnaroundHours ? Number(form.turnaroundHours) : null }); await onSaved(); } catch (exception: any) { setError(exception.response?.data?.error || "Could not add test"); } }
  return <Modal title="Add Test or Package" onClose={onClose}><form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">{error && <div className="sm:col-span-2"><ErrorBox text={error} /></div>}<Input label="Test code" required value={form.code} onChange={value => setForm({ ...form, code: value })} /><Input label="Test name" required value={form.name} onChange={value => setForm({ ...form, name: value })} /><Input label="Category" required value={form.category} onChange={value => setForm({ ...form, category: value })} /><Input label="Sample / service type" required value={form.sampleType} onChange={value => setForm({ ...form, sampleType: value })} /><Input label="Price" type="number" required value={form.price} onChange={value => setForm({ ...form, price: value })} /><Select label="Price type" value={form.priceType} onChange={value => setForm({ ...form, priceType: value })} options={[["fixed", "Fixed price"], ["starting", "Starting price"]]} /><Input label="TAT in hours (optional)" type="number" value={form.turnaroundHours} onChange={value => setForm({ ...form, turnaroundHours: value })} /><label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={form.homeCollectionAllowed} onChange={event => setForm({ ...form, homeCollectionAllowed: event.target.checked })} /> Allow home sample collection</label><div className="sm:col-span-2"><Input label="Provider-confirmed preparation instructions" value={form.preparation} onChange={value => setForm({ ...form, preparation: value })} /></div><div className="flex justify-end gap-2 sm:col-span-2"><button type="button" onClick={onClose} className="h-10 rounded-md border border-slate-300 px-4 text-sm font-semibold">Cancel</button><button className="h-10 rounded-md bg-teal-700 px-5 text-sm font-semibold text-white">Add test</button></div></form></Modal>;
}
