...
import {
  Activity, Banknote, Beaker, CalendarDays, ClipboardList, Download,
...
type Tab = "overview" | "orders" | "patients" | "samples" | "reports" | "tests" | "referrals" | "inbox";
type Test = { _id: string; code: string; name: string; category: string; sampleType: string; price: number; turnaroundHours: number; preparation: string; active: boolean };
...
type Order = {
  ...
  payment: { subtotal: number; discount: number; total: number; paid: number; status: string; method?: string };
...
function InboxView({ conversations, selected, messages, onSelect, onSent, onError }: { conversations: Conversation[]; selected: Conversation | null; messages: Message[]; onSelect: (row: Conversation) => void; onSent: () => Promise<void>; onError: (text: string) => void }) {
  ...
  return <div className="grid min-h-[650px] overflow-hidden rounded-lg border border-slate-200 bg-white lg:grid-cols-[340px_minmax(0,1fr)]"><div className="border-r border-slate-200"><div className="border-b border-slate-200 px-4 py-3"><h2 className="font-bold">Patient conversations</h2><p className="text-xs text-slate-500">WhatsApp Business inbox</p></div><div className="max-h-[590px] overflow-y-auto">{conversations.map(row => <button key={row.conversationId} onClick={() => void onSelect(row)} className={`w-full border-b border-slate-100 p-4 text-left hover:bg-slate-50 ${selected?.conversationId === row.conversationId ? "bg-green-50" : ""}`}><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-100 text-green-700"><UserRound className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{row.patientName || `+${row.phone}`}</p><p className="truncate text-xs text-slate-500">{row.latestMessage}</p><p className="mt-1 text-[11px] text-slate-400">{row.messageCount} messages · {dateTime(row.latestAt)}</p></div></div></button>)}{!conversations.length && <Empty icon={MessageCircle} text="No WhatsApp conversations yet" />}</div></div><div className="flex min-h-0 flex-col bg-slate-50">{selected ? <><div className="border-b border-slate-200 bg-white px-5 py-3"><p className="font-semibold">{selected.patientName || `+${selected.phone}`}</p><p className="text-xs text-slate-500">+{selected.phone}</p></div><div className="flex-1 space-y-3 overflow-y-auto p-5">{messages.map(message => <div key={message._id} className={`flex ${message.sentBy === "patient" ? "justify-start" : "justify-end"}`}><div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm shadow-sm ${message.sentBy === "patient" ? "border border-slate-200 bg-white" : "bg-green-600 text-white"}`}><p className="whitespace-pre-wrap">{message.message || message.documentName || `[${message.type}]`}</p><p className={`mt-1 text-right text-[10px] ${message.sentBy === "patient" ? "text-slate-400" : "text-green-100"}`}>{dateTime(message.createdAt)}</p></div></div>)}</div><form onSubmit={submit} className="flex gap-2 border-t border-slate-200 bg-white p-4"><input value={text} onChange={event => setText(event.target.value)} placeholder="Reply to patient" className="h-10 flex-1 rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-green-600" /><button disabled={sending || !text.trim()} className="inline-flex h-10 items-center gap-2 rounded-md bg-green-600 px-4 text-sm font-semibold text-white disabled:opacity-50"><Send className="h-4 w-4" /> Send</button></form></> : <div className="flex flex-1 items-center justify-center"><Empty icon={MessageCircle} text="Select a patient conversation" /></div>}</div></div>;
}
...
function BookingModal({ tests, referrals, onClose, onSaved }: { tests: Test[]; referrals: Referral[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const total = tests.filter(test => selectedTests.includes(test._id)).reduce((sum, test) => sum + test.price, 0);
...
function TestModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState({ code: "", name: "", category: "Pathology", sampleType: "Blood", price: "", turnaroundHours: "24", preparation: "" }); const [error, setError] = useState("");
  async function submit(event: FormEvent) { event.preventDefault(); try { await pathologyAPI.createTest({ ...form, price: Number(form.price), turnaroundHours: Number(form.turnaroundHours) }); await onSaved(); } catch (exception: any) { setError(exception.response?.data?.error || "Could not add test"); } }
  return <Modal title="Add Test or Package" onClose={onClose}><form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">{error && <div className="sm:col-span-2"><ErrorBox text={error} /></div>}<Input label="Test code" required value={form.code} onChange={value => setForm({ ...form, code: value })} /><Input label="Test name" required value={form.name} onChange={value => setForm({ ...form, name: value })} /><Input label="Category" required value={form.category} onChange={value => setForm({ ...form, category: value })} /><Input label="Sample type" required value={form.sampleType} onChange={value => setForm({ ...form, sampleType: value })} /><Input label="Price" type="number" required value={form.price} onChange={value => setForm({ ...form, price: value })} /><Input label="TAT in hours" type="number" required value={form.turnaroundHours} onChange={value => setForm({ ...form, turnaroundHours: value })} /><div className="sm:col-span-2"><Input label="Preparation instructions" value={form.preparation} onChange={value => setForm({ ...form, preparation: value })} /></div><div className="flex justify-end gap-2 sm:col-span-2"><button type="button" onClick={onClose} className="h-10 rounded-md border border-slate-300 px-4 text-sm font-semibold">Cancel</button><button className="h-10 rounded-md bg-teal-700 px-5 text-sm font-semibold text-white">Add test</button></div></form></Modal>;
}
