...
type WhatsAppAiForm = {
  ...
  pathologyDoctorContacts: Array<{ name: string; phone: string; active: boolean }>;
  pathologyEmergencyAlertsEnabled: boolean; pathologyEmergencyOwnerPhone: string;
  pathologyEmergencySampleTemplate: string; pathologyEmergencyReportTemplate: string; pathologyEmergencyTemplateLanguage: string;
};
...
const emptyWhatsAppAiForm: WhatsAppAiForm = {
  ...
  pathologyDoctorContacts: [], pathologyEmergencyAlertsEnabled: false, pathologyEmergencyOwnerPhone: "",
  pathologyEmergencySampleTemplate: "pathology_emergency_sample_alert", pathologyEmergencyReportTemplate: "pathology_emergency_report_alert", pathologyEmergencyTemplateLanguage: "hi",
};
...
function WhatsAppAiSetup({ onNotice }: { onNotice: (type: "ok" | "error", text: string) => void }) {
  ...
  const customPromptWordCount = countWords(form.customPrompt);
  ...
  const updateDoctorContact = (index: number, key: "name" | "phone", value: string) => setForm(current => ({ ...current, pathologyDoctorContacts: current.pathologyDoctorContacts.map((contact, rowIndex) => rowIndex === index ? { ...contact, [key]: value } : contact) }));
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <Panel><div className="border-b border-slate-200 p-5"><h2 className="font-bold">Gemini AI behavior</h2><p className="text-sm text-slate-500">Your prompt customizes the bot but cannot override medical-safety, privacy, or tenant-isolation rules.</p></div><div className="space-y-4 p-5"><Input label="Welcome message" value={form.welcomeMessage} onChange={value => set("welcomeMessage", value)} /><label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-600">Custom pathology prompt</span><textarea rows={10} value={form.customPrompt} onChange={event => { const value = event.target.value; if (countWords(value) <= PATHOLOGY_CUSTOM_PROMPT_WORD_LIMIT) set("customPrompt", value); }} placeholder="Describe your lab, supported languages, collection rules, tone, special instructions, and when staff should take over..." className="w-full rounded-md border border-slate-300 p-3 text-sm outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-100" /><span className="mt-1 block text-right text-xs text-slate-400">{customPromptWordCount}/{PATHOLOGY_CUSTOM_PROMPT_WORD_LIMIT} words</span></label><Input label="Gemini model override (optional)" value={form.aiModel} onChange={value => set("aiModel", value)} /></div></Panel>
    <Panel><div className="flex items-center justify-between gap-3 border-b border-slate-200 p-5"><div><h2 className="font-bold">Doctor recognition and emergency alerts</h2><p className="text-sm text-slate-500">Recognize doctors by WhatsApp number and forward emergency sample/report alerts to the owner.</p></div><button type="button" onClick={() => set("pathologyDoctorContacts", [...form.pathologyDoctorContacts, { name: "", phone: "", active: true }])} className="inline-flex h-9 shrink-0 items-center gap-1 rounded-md border border-slate-300 px-3 text-xs font-semibold hover:bg-slate-50"><Plus className="h-3.5 w-3.5" /> Add doctor</button></div><div className="space-y-4 p-5"><div className="grid gap-4 sm:grid-cols-2"><Input label="Emergency owner WhatsApp number" value={form.pathologyEmergencyOwnerPhone} onChange={value => set("pathologyEmergencyOwnerPhone", value)} /><label className="flex items-center gap-2 self-end pb-3 text-sm font-medium"><input type="checkbox" checked={form.pathologyEmergencyAlertsEnabled} onChange={event => set("pathologyEmergencyAlertsEnabled", event.target.checked)} /> Enable emergency owner alerts</label></div><div className="space-y-3">{form.pathologyDoctorContacts.map((contact, index) => <div key={index} className="grid gap-3 rounded-md border border-slate-200 p-3 sm:grid-cols-[1fr_1fr_auto]"><Input label="Doctor name" value={contact.name} onChange={value => updateDoctorContact(index, "name", value)} /><Input label="Doctor WhatsApp number" value={contact.phone} onChange={value => updateDoctorContact(index, "phone", value)} /><button type="button" aria-label={`Remove ${contact.name || "doctor"}`} onClick={() => set("pathologyDoctorContacts", form.pathologyDoctorContacts.filter((_, rowIndex) => rowIndex !== index))} className="mt-5 h-10 rounded-md border border-rose-200 px-3 text-rose-700 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button></div>)}{!form.pathologyDoctorContacts.length && <p className="rounded-md bg-slate-50 px-4 py-3 text-sm text-slate-500">No doctor numbers configured yet.</p>}</div><p className="text-xs text-slate-500">Emergency Report flow asks only for the patient name. Generic “Emergency” messages alert the owner immediately.</p></div></Panel>
    ...
    <Panel><div className="border-b border-slate-200 p-5"><h2 className="font-bold">Emergency WhatsApp templates</h2><p className="text-sm text-slate-500">Use approved Meta templates so owner alerts can be delivered outside the 24-hour conversation window.</p></div><div className="grid gap-4 p-5 sm:grid-cols-2"><Input label="Emergency sample template" value={form.pathologyEmergencySampleTemplate} onChange={value => set("pathologyEmergencySampleTemplate", value)} /><Input label="Emergency report template" value={form.pathologyEmergencyReportTemplate} onChange={value => set("pathologyEmergencyReportTemplate", value)} /><Input label="Template language code" value={form.pathologyEmergencyTemplateLanguage} onChange={value => set("pathologyEmergencyTemplateLanguage", value)} /><div className="rounded-md bg-slate-50 p-3 text-xs text-slate-600 sm:col-span-2"><p><b>Sample body:</b> ⚠️ इमरजेंसी अलर्ट: &#123;&#123;1&#125;&#125; की तरफ से इमरजेंसी सैंपल का मैसेज आया है। कृपया तुरंत ध्यान दें!</p><p className="mt-2"><b>Report body:</b> 🚨 इमरजेंसी अलर्ट: डॉक्टर &#123;&#123;1&#125;&#125; ने &#123;&#123;2&#125;&#125; की इमरजेंसी रिपोर्ट की मांग की है। कृपया तुरंत ध्यान दें।</p></div></div></Panel>
