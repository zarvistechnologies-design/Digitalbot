  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <button
        ...
        className="fixed left-4 top-4 z-40 rounded-xl border border-slate-200 bg-white p-3 shadow-sm lg:hidden"
      ...
      <main className="flex-1 px-4 pb-12 pt-20 sm:px-6 lg:ml-64 lg:px-8 lg:pt-8">
        <div className="mx-auto max-w-6xl">
          <header className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-teal-700">Appointments</p>
              <h1 className="text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Share appointment schedule</h1>
              <p className="mt-2 text-sm font-medium text-slate-500">Give a doctor one private link to their live upcoming appointment list.</p>
            </div>
            <div className="inline-flex w-fit items-center gap-2 rounded-md border border-teal-200 bg-teal-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-teal-700">
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(340px,0.95fr)]">
            <section className="rounded-xl border border-slate-200 bg-white">
              ...
              <div className="p-5 sm:p-6">
                {message && (
                  <div className="mb-5 flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800" role="status">
                    <CheckCircle2 className="h-5 w-5 shrink-0" /> {message}
                  </div>
                )}
                {(error || doctorsError) && (
                  <div className="mb-5 flex items-center gap-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-800" role="alert">
                ...
                <label className="block">
                  <span className="mb-2 block text-[11px] font-bold uppercase tracking-wide text-slate-500">Doctor</span>
                  <div className="relative">
                    ...
                    <select
                      ...
                      disabled={isPending}
                      className="h-12 w-full appearance-none rounded-lg border border-slate-200 bg-white pl-12 pr-10 text-sm font-semibold text-slate-800 outline-none transition focus:border-teal-500 focus:ring-1 focus:ring-teal-500 disabled:bg-slate-50"
                    >
                ...
                <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500">Selected doctor</p>
                ...
                <button
                  ...
                  className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-teal-600 px-6 text-sm font-bold text-white transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                {copiedUrl && (
                  <div className="mt-4 flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <p className="min-w-0 flex-1 truncate text-xs font-mono font-medium text-slate-600">{copiedUrl}</p>
                    <a
                      href={copiedUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-700 transition hover:border-teal-200 hover:bg-teal-50 hover:text-teal-700"
                    >
            ...
            <aside className="rounded-xl bg-slate-900 p-4 sm:p-5">
              ...
              <div className="overflow-hidden rounded-lg bg-white ring-1 ring-white/10">
                <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
                  <span className="text-sm font-black tracking-tight text-slate-900">Digital<span className="text-teal-600">Bot</span>.AI</span>
                ...
                <div className="p-4">
                  <div className="rounded-lg bg-slate-900 p-4 text-white">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Upcoming appointments</p>
                    <p className="mt-1 text-lg font-bold">{selectedDoctor ? `Dr. ${selectedDoctor.name}` : "Doctor name"}</p>
                  </div>

                  <div className="mt-3 overflow-hidden rounded-lg border border-slate-200 bg-white">
                    <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-slate-500">
                      <CalendarDays className="h-3.5 w-3.5 text-teal-600" /> Today&apos;s schedule
                    </div>
                    {["10:30 AM", "11:00 AM", "11:30 AM"].map((time, index) => (
                      <div key={time} className="grid grid-cols-[26px_minmax(0,1fr)_auto] items-center gap-2 border-b border-slate-100 px-3 py-2.5 last:border-b-0">
                        <span className="flex h-6 w-6 items-center justify-center rounded-md bg-slate-100 text-[10px] font-bold font-mono text-slate-500">{index + 1}</span>
                        <div className="min-w-0">
                          <p className="truncate text-xs font-bold text-slate-800">Patient name</p>
                          <p className="mt-0.5 flex items-center gap-1 text-[10px] font-mono text-slate-400"><Phone className="h-2.5 w-2.5" /> +91 ••••• ••{21 + index}</p>
                        </div>
                        <span className="rounded-md bg-teal-50 border border-teal-200 px-2 py-1 text-[10px] font-bold font-mono text-teal-700">{time}</span>
                      </div>
                    ))}
                  </div>
                </div>
